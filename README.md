...
A data frame goes in, data frames and their metadata come out. Which means
the numbers are the same, whichever language you call them from.
...
megabytes and a network connection, once. Nothing is installed
...
of card4r therefore run the same code, and `card_config()` says which:
...
over the hydrological year starting in September, `VCN10` starts its year
at the month of highest flows, February here.
...
[in French](https://lou-heraut.github.io/card/CARDS.fr). From R:
...
`output = "série"`). `statistic` and `aspect` are orthogonal: `VCN10` and
`tVCN10` are both a `minimum`, one being its value and the other its
date.
...
top of the [catalogue](https://lou-heraut.github.io/card/CARDS) and given
in full in
...
takes to cite it or to replay it years later. `card_config()` prints the
same without running a computation.
...
same series in this package's test suite, so the agreement is measured at
every release rather than claimed here.
